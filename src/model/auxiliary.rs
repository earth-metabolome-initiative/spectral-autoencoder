...
pub struct AuxiliaryLossConfig {
    ...
    /// Number of predicted peak slots randomly sampled per forward pass by the
    /// Chamfer m/z magnet. `0` disables subsampling and evaluates every pred
    /// slot (default). Smaller values reduce the broadcast tensor's footprint
    /// proportionally while keeping each sampled slot's `min`-over-targets
    /// exact, so the loss remains an unbiased estimator of the full mean.
    #[serde(default = "default_chamfer_max_pred_peaks")]
    pub chamfer_max_pred_peaks: usize,
}
...
impl Default for AuxiliaryLossConfig {
    fn default() -> Self {
        Self {
            ...
            chamfer_max_pred_peaks: DEFAULT_CHAMFER_MAX_PRED_PEAKS,
        }
    }
}
...
/// Default count of pred slots randomly sampled by the Chamfer m/z magnet.
/// `0` disables subsampling (every slot is evaluated), matching the original
/// kernel behavior.
pub const DEFAULT_CHAMFER_MAX_PRED_PEAKS: usize = 0;

fn default_chamfer_max_pred_peaks() -> usize {
    DEFAULT_CHAMFER_MAX_PRED_PEAKS
}
...
impl AuxiliaryLossConfigBuilder {
    ...
    /// Sets the count of pred slots randomly sampled by the Chamfer m/z
    /// magnet on each forward pass. `0` disables subsampling (evaluates
    /// every slot); smaller values shrink the broadcast tensor and keep
    /// the loss unbiased.
    #[inline]
    #[must_use]
    pub fn with_chamfer_max_pred_peaks(mut self, value: usize) -> Self {
        self.config.chamfer_max_pred_peaks = value;
        self
    }
