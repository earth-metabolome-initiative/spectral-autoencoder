...
default preprocessing keeps 60 peaks per spectrum. Optional metadata is encoded
with explicit unknown buckets so the same models can run when precursor or
instrument fields are missing.
...
- `SpectralAutoencoderConfig::twenty_million_run()` is the flat-vector model:
  60 `(m/z, intensity)` pairs, deep MLP encoder/decoder, 64-dimensional latent,
  and 6,098,618 parameters.
- `PeakSetAutoencoderConfig::twenty_million_run()` is the peak-set model:
  60 masked peak tokens, transformer encoder, learned-query set decoder,
  64-dimensional latent, and 39,086,149 parameters.
...
- retention-order prediction for same-file spectra with `RTINSECONDS`
...
The examples use mascot-rs' GeMS-A10 top-60 Zenodo loader. Data is cached under
`datasets/gems-a10-top-60-peaks` by default. The flat-vector example also keeps
a persistent preprocessed CPU cache so restarts can skip repeated vectorization.

Run the flat-vector model:

```bash
GEMS_RUN_DIR=runs/gems-a10-top60-flat-cache100-bs1024-10epoch \
...
```bash
GEMS_RUN_DIR=runs/gems-a10-top60-peak-cache80-bs128-5epoch \
